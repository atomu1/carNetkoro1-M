...
- (void)viewDidLoad {
    ...
    self.priceLabel.text=[NSString stringWithFormat:@"%@",self.orderModal.GaragePrice];
    [self.payButton setTitle:[NSString stringWithFormat:@"确认支付%@元",self.orderModal.GaragePrice] forState:UIControlStateNormal];
...
-(void)gotoAlipay:(MaintainModel*)orderModal{
    [self getAlipayOrderInfoFromNetWork];
}

-(void)gotoWxchatPay:(MaintainModel*)orderModal{
...
-(void)getAlipayOrderInfoFromNetWork
{
    ...
    NSDictionary *paramDict = @{
                                @"subject":self.orderModal.GarageName,
                                @"body":@"订单",
                                @"price":self.orderModal.GaragePrice,
                                @"out_trade_no":self.orderModal.GarageOrdersId
                                };
                                          ...
                                          if ([jsonDic objectForKey:@"Success"]) {
                                              ...
                                              [[AlipaySDK defaultService] payOrder:returnData fromScheme:kAliPayURLScheme callback:^(NSDictionary *resultDic) {
                                                  ...
                                                  if ([resultDic[@"resultStatus"] intValue]==9000) {
                                                      ...
                                                      //请求成功的本地回调接口
                                                      [self postAlipayOrderStateToNetWork:self.orderModal.GarageOrdersId];
                                                  }
                                                  else{
